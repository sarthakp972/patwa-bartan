...
const HomeViewAll = () => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        if (snapshot.exists()) {
          ...
          const productsArray = Object.entries(data).map(([id, product]) => ({ id, ...product }));
  ...
  return (
    <Container className="home-view-all">
      <h2 className="section-title">{t("home_special_products")}</h2>
      {loading && (
            ...
            <Card className="product-card" onClick={(e) => handleCardClick(e, product)}>
              <div className="product-content">
                <div className="product-image-container">
                  {product.discount && (
                    <div className="sale-badge">{t("home_sale_badge")}: {product.discount}%</div>
                  )}
